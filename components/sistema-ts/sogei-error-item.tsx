...
export function SogeiErrorItem({
  ...
  if (helper.gravita === "INFO") {
    return (
      <div className="rounded-md bg-amber-500/10 p-2.5 border border-amber-500/30 text-xs text-amber-900 dark:text-amber-200 space-y-1.5 whitespace-normal break-words">
        ...
        {showGuidance && helper.azioneConsigliata && (
          <div className="mt-1 pt-1.5 border-t border-amber-500/20 text-xs flex items-start gap-1.5 text-amber-800 dark:text-amber-300">
  ...
  if (helper.gravita === "WARNING") {
    return (
      <div className="rounded-md bg-amber-50 dark:bg-amber-950/20 p-2.5 border border-amber-300 dark:border-amber-800/50 text-xs text-amber-900 dark:text-amber-200 space-y-1.5 whitespace-normal break-words">
        <div className="font-semibold flex items-center gap-1.5 text-amber-700 dark:text-amber-400">
        ...
        {showGuidance && helper.azioneConsigliata && (
          <div className="mt-1 pt-1.5 border-t border-amber-200 dark:border-amber-900/40 text-xs flex items-start gap-1.5 text-amber-800 dark:text-amber-300">
            <span className="font-semibold shrink-0">Cosa fare:</span>
            <span className="min-w-0 flex-1 break-words">{helper.azioneConsigliata}</span>
          </div>
        )}
  ...
  return (
    <div className="rounded-md bg-destructive/10 p-2.5 border border-destructive/25 text-xs text-foreground space-y-1.5 whitespace-normal break-words">
      ...
      {showGuidance && helper.azioneConsigliata && (
        <div className="mt-1 pt-1.5 border-t border-destructive/20 text-xs flex items-start gap-1.5 text-foreground/90">
