...
export function SogeiErrorBadge({
  ...
  const chipClasses = cn(
    "inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-medium border transition-colors cursor-pointer select-none text-left focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-primary",
    isInfo &&
  ...
  return (
    <Popover>
      ...
      >
        {isInfo && <Info className="h-3 w-3 shrink-0" aria-hidden="true" />}
        {isWarning && <AlertCircle className="h-3 w-3 shrink-0" aria-hidden="true" />}
        ...
        <span className="truncate max-w-[170px] sm:max-w-[220px]">{helper.etichetta}</span>
      </PopoverTrigger>
      ...
      <PopoverContent
        ...
        align={align}
        sideOffset={6}
        className="w-80 sm:w-96 p-3 space-y-2 text-xs shadow-lg bg-popover text-popover-foreground border-border"
      >
        {/* Intestazione */}
        <div
          className={cn(
            "font-semibold flex items-start gap-1.5 text-xs pb-1.5 border-b border-border/60",
            isInfo && "text-blue-700 dark:text-blue-400",
            isWarning && "text-amber-700 dark:text-amber-400",
            isError && "text-destructive"
          )}
        >
          {isInfo && <Info className="h-3.5 w-3.5 shrink-0 mt-0.5" />}
          {isWarning && <AlertCircle className="h-3.5 w-3.5 shrink-0 mt-0.5" />}
          {isError && <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5" />}
        ...
        <div className="text-[11px] text-muted-foreground flex items-start gap-1.5 bg-muted/50 p-2 rounded border border-border/40">
        ...
        {showGuidance && helper.azioneConsigliata && (
          <div className="pt-1.5 border-t border-border/60 text-[11px] flex items-start gap-1.5">
            <ArrowRight
              className={cn(
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="text-foreground/90">{helper.azioneConsigliata}</span>
            </div>
          </div>
        )}
      </PopoverContent>
