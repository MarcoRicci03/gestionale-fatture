...
- `npm run verify:rich-text` — verifica il round-trip testo↔rich-content dell'editor blocchi PDF (`scripts/verify-rich-text-bridge.ts`). Non esiste un test runner (jest/vitest/playwright) configurato.
- `npm run verify:actions-auth` — verifica che ogni Server Action esportata da `lib/actions/*.ts` chiami `requireUserId`/`requireSession`/`requireAdmin` (`scripts/verify-actions-auth.ts`); vedi nota su `proxy.ts` sotto.
