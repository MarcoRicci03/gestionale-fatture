...
import { buildPatientWhere } from "./list-query";

describe("buildPatientWhere", () => {
  it("senza ricerca: solo id_Utente e archiviato", () => {
    const where = buildPatientWhere(7, { search: "", archiviato: false });
    expect(where).toEqual({ AND: [{ id_Utente: 7 }, { archiviato: false }] });
  });

  it("archiviato: true per la lista archiviati", () => {
    const where = buildPatientWhere(7, { search: "", archiviato: true });
    expect(where).toEqual({ AND: [{ id_Utente: 7 }, { archiviato: true }] });
  });

  it("ricerca con un token: OR tra cognome e nome", () => {
    const where = buildPatientWhere(7, { search: "Rossi", archiviato: false });
  ...
  it("ricerca con due token: ogni token deve comparire (AND di due OR)", () => {
    const where = buildPatientWhere(7, { search: "Mario Rossi", archiviato: false });
