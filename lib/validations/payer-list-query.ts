...
// (vedi PayersManager), ma condividono lo stesso testo di ricerca `q`.
export function parsePayerListQuery(raw: RawSearchParams): {
  search: string;
  page: number;
  archivedPage: number;
  pageSize: number;
  ...
  const pageSize = parsePageSize(firstValue(raw.pageSize), PAYERS_PAGE_SIZE);
